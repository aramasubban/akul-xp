<template>
  <section class="text-xs md:text-header-window">
    <div class="flex w-full text-xxs md:text-xs items-center justify-center md:justify-start ml-px flex-wrap gap-4 md:min-w-80 md:ml-1">
      <div class="flex flex-col items-center flex-wrap">
        <img :src="pub('/img/icons/projects/tools/truenas.svg')" :alt="$t('common.icon') + ' truenas'" class="w-9 h-9" />
        <p class="font-bold">TrueNas Scale</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="pub('/img/icons/projects/tools/kubernetes.svg')" :alt="$t('common.icon') + ' kubernetes'" class="w-9 h-9" />
        <p class="font-bold">Kubernetes</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="pub('/img/icons/projects/tools/docker.svg')" :alt="$t('common.icon') + ' docker'" class="w-10 h-10" />
        <p class="font-bold mb-px">Docker</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="pub('/img/icons/projects/tools/bash.svg')" :alt="$t('common.icon') + ' bash'" class="w-9 h-9" />
        <p class="font-bold">Bash</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="pub('/img/icons/projects/tools/cpu.svg')" :alt="$t('common.icon') + ' cpu'" class="w-9 h-9" />
        <p class="font-bold">Hardware</p>
      </div>
      <div class="flex flex-col items-center flex-wrap">
        <img :src="pub('/img/icons/projects/tools/jenkins.svg')" :alt="$t('common.icon') + ' jenkins'" class="w-9 h-9" />
        <p class="font-bold mt-px">CI / CD</p>
      </div>
    </div>
    <video controls autoplay muted loop width="400" class="mt-5">
      <source :src="pub('/videos/montage-serveur.mp4')" type="video/mp4" />
      {{ $t('common.errorVideo') }}
    </video>
  ...
  >
    <img :src="pub('/img/icons/projects/tools/web.svg')" :alt="$t('buttons.linkWebSite') + ' ' +  ProjectName" class="w-3 h-3 mr-0.5"/>
